...
### Родитель — мобильный [`docs/c4/journey_parent_mobile.puml`](docs/c4/journey_parent_mobile.puml)

7 потоков мобильного сценария:

- **Установка**: получение ссылки-приглашения → активация аккаунта → разрешение Push
...
- **Календарь**: переключение месяц/список, deep link на тренировку / матч / турнир
- **Статистика ребёнка**: посещаемость + матчевая статистика по периодам
- **Live-уведомления**: гол → итог матча → deep link на live-экран
- **Настройки**: управление push/Telegram-уведомлениями по типам

### Игрок — мобильный [`docs/c4/journey_player_mobile.puml`](docs/c4/journey_player_mobile.puml)
...
| 2 — Тренировки | 4 нед. | Расписание + Cron + RSVP + авто-травма + Push |
| 3 — Матчи и турниры | 5 нед. | Все типы матчей + live-таймер + push на гол |
| 4 — Статистика и экспорт | 2 нед. | Отчёты + Excel-экспорт + iCal |
| **MVP Web** | **~19 нед.** | **Веб-платформа готова к пилоту** |
| 5 — Мобильное приложение | 8 нед. | React Native/Flutter: RSVP + live + push + offline |
| **MVP Mobile** | **~27 нед.** | **App Store + Google Play** |
